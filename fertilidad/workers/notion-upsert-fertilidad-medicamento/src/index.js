...
function validate(body) {
  ...
  const b = body;
  for (const f of ["client_id", "nombre"]) {
...
async function findByClientId(env, clientId) {
  const res = await notionRequest(
    env.NOTION_TOKEN,
    `/databases/${env.NOTION_MEDICAMENTOS_DB_ID}/query`,
    {
      method: "POST",
      body: JSON.stringify({
...
function buildProperties(p) {
  const title = `${p.nombre}${p.dosis ? ` ${p.dosis}` : ""}${p.frecuencia ? ` \xB7 ${p.frecuencia}` : ""}`;
  const props = {
    Nombre: { title: [{ type: "text", text: { content: title.slice(0, 200) } }] },
    "Client ID": richText(p.client_id)
  };
  if (p.dosis) props["Dosis"] = richText(p.dosis);
  if (p.frecuencia) props["Frecuencia"] = richText(p.frecuencia);
  if (p.via) props["V\xEDa"] = { select: { name: p.via } };
...
var index_default = {
  async fetch(req, env) {
    ...
    if (!env.NOTION_MEDICAMENTOS_DB_ID)
      return jsonResponse({ error: "missing_secret", detail: "NOTION_MEDICAMENTOS_DB_ID" }, 500);
    if (req.method === "GET") {
      return jsonResponse({
        worker: "notion-upsert-fertilidad-medicamento",
        usage: "POST con JSON { client_id, nombre, ... }. Idempotente por client_id."
      });
    }
    ...
    try {
      ...
      const created = await notionRequest(
        ...
        "/pages",
        {
          method: "POST",
          body: JSON.stringify({
            parent: { database_id: env.NOTION_MEDICAMENTOS_DB_ID },
            properties: props
          })
        }
      );
